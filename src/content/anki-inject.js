...
const triggerInput = function(el) { 
	const event = new Event("input", { bubbles: true });
	return el.dispatchEvent(event);
};

const getAnkiInfo = (ankiSavedWord, ankiSkippedWord) => chrome.runtime.sendMessage({
    type: 'get anki info',
    ankiSavedWord,
    ankiSkippedWord
}, async function(res) {
    ...
    $('.field:eq(0), .field:eq(1)').on('input', debounce((e => $('img', e.target).each(async function() {
        const $img = $(this);
        if ($img.attr('is-handling')) { return; }
        ...
        triggerInput($('.field:eq(0)')[0]);
        return triggerInput($('.field:eq(1)')[0]);})), 1000)
    );

    return $('.field').on('click', '.dictionariez-anki-image-close', function(e) {
...
<button class="btn btn-secondary btn-skip mt-2" style="float: right">Skip</button>`;
	...
	return $('.btn-skip').on('click', function() {
		$('.field:eq(0)').empty();
		$('.field:eq(1)').empty(); 
		return getAnkiInfo(null, currentWordItem.w);
	});
};

const wordSavedCallback = async function(ev) {
	if (!ev.target.classList.contains('btn-primary')) { return; }
	if (!currentWordItem?.w) { return; }

	try {
		await utils.checkInTime(() => $('.field:eq(0)').text().trim() === '');

		return getAnkiInfo(currentWordItem.w);
	} catch (error) { 
		return console.error(`Anki save failed on word: ${currentWordItem.w}`);
	}
};

var renderLookupInfo = (wordItem, followedWords) => (function(lookupInfo) {
    ...
    if (lookupInfo?.w) {
        ...
        return $('.field:eq(1)').append(renderLookupWords(wordItem, lookupInfo));
    }
});
			
const replaceW = (text, w) => text.replaceAll(w, "<span style='font-weight: bold'>[?]</span>")
...
var renderImages = function(images) {
	const imgs = images.map(function(cur, i) {
		if (cur.dataUrl && (i < 2)) {  // only display two images
...
var renderLookupDefs = function(res, followedWords = []) {
	let pronHtml;
	const defTpl = def => `<span class='fairydict-def' style='display: inline-flex;margin-bottom: 3px;'> ${def} </span>`;
	const defsTpl = defs => `<span class='fairydict-defs' style='display: table-cell;padding-top: 1px;'> ${defs} </span>`;
	const labelsTpl = labels => `<div class='fairydict-labels' style='margin: 8px 0px;'> ${labels} </div>`;
	const labelTpl = label => `<span class='fairydict-label' style='border: 1px solid;border-radius: 3px;padding: 1px 2px;margin-right: 3px;border-color: dimgray;'> ${label} </span>`;
	const posTpl = pos => `<span class='fairydict-pos' style='display: table-cell;width: 40px;padding-top: 1px;'> ${pos} </span>`;
	const contentTpl = content => `<div class='fairydict-content' style='font-size: 15px;line-height: 15px; padding: 0 5px; border-left:5px solid gold;'> ${content} </div>`;
	const pronSymbolTpl = (symbol='', type='') => `<span class='fairydict-symbol fairydict-symbol-${type}'> <em> ${symbol} </em> </span>`;
	const pronAudioTpl = (w, src='', type='', synthesis='') => `<a class='fairydict-pron-audio fairydict-pron-audio-${type}' data-mp3='${src}' data-synthesis='${synthesis}' data-w='${w}'><i class='icon-fairydict-volume'></i></a>`;
	...
	if (pronHtml) { html += pronsTpl(pronHtml); }

	const renderDef = function(def) {
		if (def) { return defTpl(followedWords.reduce(((res, fw) => res.replace(fw, '[?]')), def)
		); }
	};
	...
	const renderItem = function(item) {
		...
		const labelsCon = (item.labels || []).map(function(name) { if (name) { return labelTpl(name); } }).reduce((function(prev, cur) {
			if (cur) { prev += cur; } 
			return prev;
		}), '');
		...
		const defsCon = defs.map(function(def, i) { 
			if (def) { 
				if (defs.length === 1) { 
			...
			if (next) { return prev + '<br>' + next; } 
			return prev;
		}); 
			
		if (defsCon) { defsHtml = defsTpl(defsCon); }

		if (defsHtml) { return html += contentTpl(posHtml + labelsHtml + defsHtml); }
	};
...
var renderLookupWords = function(wordItem, res) {
	const wTpl = (w='', w2='') => `<strong class='fairydict-w' style='font-size: 20px;'> ${w} </strong> &nbsp; <span style='font-size: 13px;'>${w2}</span>`;
	const pronSymbolTpl = (symbol='', type='') => `<span class='fairydict-symbol fairydict-symbol-${type}'> <em> ${symbol} </em> </span>`;
	const pronAudioTpl = (w, src='', type='', synthesis='') => `<a class='fairydict-pron-audio fairydict-pron-audio-${type}' data-mp3='${src}' data-synthesis='${synthesis}' data-w='${w}'><i class='icon-fairydict-volume'></i></a>`;
	...
	if (res?.prons && res.w) { 
		pronHtml = res.prons.reduce((function(prev, cur){
			if (cur.synthesis || cur.audio || cur.symbol) {
				...
				if (cur.synthesis || cur.audio) { prev += pronAudioTpl(res.w.replaceAll('·', ''), cur.audio, cur.type, cur.synthesis); }
			}
			return prev;
		}), '');
	...
	if (pronHtml || wHtml) { return pronsTpl(wHtml, pronHtml); }
};
	...
	return (() => {
		...
		for (var item of prons) { 
			...
			if ((item.type === 'bre') && item.audio) {
				result.push($('.field .fairydict-pron-audio-bre').attr('data-mp3', item.audio));
			} else {
...
export var initAnkiInjection = function() { 
	if (['https://ankiweb.net', 'https://ankiuser.net'].includes(location.origin) && (location.pathname === '/add')) {
		...
		return window.addEventListener('beforeunload', () => utils.send('beforeunload anki window', {
            left: window.screenX,
            top: window.screenY,
