...
const setEnglishProns = result => result.prons = result.prons.concat([
    {
    "symbol": "UK",
    "type": "bre",
    "synthesis": "en-GB"
...
export default ({
    ...
    checkType(w) {
        ...
        for (var name in parserDescs) {
            ...
            if (dictDesc.supportChinese) {
                if (utils.isChinese(w) && setting.getValue("enableLookupChinese")) { return name; }
            }
            
            if (dictDesc.languages) {
                for (var lang of dictDesc.languages) { 
                    if ((w.match(new RegExp(langs[lang].regex, 'ug'))?.length === w.length) && !this.isLangDisabled(lang)) {
                        return name;
                    }
                }
            }
    ...
    init() {
        ...
        for (var dictDesc of Object.values(parserDescs)) {
            dictDesc.languages?.forEach(n => {
                if (process.env.PRODUCT === "Ordböcker") {
                    if (n === 'Swedish') {
                        if (!this.otherSupportedLanguages.includes(n)) { return this.otherSupportedLanguages.push(n); }
                    } else { 
                        if (!setting.configCache.otherDisabledLanguages.includes(n)) { return setting.configCache.otherDisabledLanguages.push(n); }
                    }
                } else { 
                    if (!this.otherSupportedLanguages.includes(n)) { return this.otherSupportedLanguages.push(n); }
                }
        ...
        return message.on('get real person voice', ({ w }, sender) => {
            if (w && setting.getValue('enableRealPron')) {
                if (w.split(' ').length === 1) { return this.parse(sender.tab.id, w.replaceAll('·', ''), 'ldoce'); }
            }
        });
    },  // ignore phrase
    ...
    async parse(tabId, w, tname, prevResult, url) {
        ...
        if ((tname === 'google') && (possibleLangs.length > 0) && (possibleLangs[0] !== 'English')) {
                ...
                if (langDesc.synthesis) { url = url.replace('hl=en', 'hl=' + langDesc.synthesis); } 
            }
        ...
        try {
            ...
            if ((err.message === 'timeout') && utils.isEnglish(w) && !prevResult) {
                ...
                if ((url.includes('en.wiktionary.org')) && possibleLangs.includes('Swedish')) { 
                    return this.parse(tabId, w, 'wiktionary', prevResult, url.replace(/\w+.wiktionary.org/, 'sv.wiktionary.org'));
                } else if ((!url.includes('de.wiktionary.org')) && possibleLangs.includes('German')) {
                    return this.parse(tabId, w, 'wiktionary', prevResult, url.replace(/\w+.wiktionary.org/, 'de.wiktionary.org'));
                } else if ((!url.includes('uk.wiktionary.org')) && possibleLangs.includes('Ukrainian')) {
                    return this.parse(tabId, w, 'wiktionary', prevResult, url.replace(/\w+.wiktionary.org/, 'uk.wiktionary.org'));
        ...
        if (tname === "bingCN") {
            if (utils.isChinese(w)) { 
                ...
                if (setting.getValue('enableConvertCn2T')) { convertCn2T(result); }
            } else { 
        ...
        if (tname === 'wiktionary') {
            ...
            if (result?.langTargets?.length > 1) {
                result.langTargets.sort(function(a, b) {
                    if (['english', 'engelska'].includes(b.lang?.toLowerCase())) { return -1;
                    } else if (['english', 'engelska'].includes(a.lang?.toLowerCase())) { return 1;
                    } else { return 0; }
                });
            }
    ...
    async parseFollowWordsOnWiktionary(tabId, w, targetLang, multipleResult) { 
        ...
        const isUniqueWord = word => word && (w !== word) && multipleResult.every(n => n.w?.replaceAll('·', '') !== word);
    ...
    async parseOtherLang(tabId, w, lang, wiktionaryResult, prevResult) {
        ...
        if (prevResult) { return [prevResult, result]; } else { return result; }
    } 
});
