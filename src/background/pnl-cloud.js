...
export default {
    ...
    async ttsSpeak({ text, lang, voice }) {
        ...
        if (!response.ok) {
            ...
            const errorMessage = data?.error || data?.status?.message || response.statusText || "Unknown error";
    ...
    async lookupInAI({ word, sentence, detectedLangInContext, userLanguage }) {
        const response = await fetch(pnlBase + "/api/lookup-in-ai", {
            ...
            body: JSON.stringify({ word, sentence, detectedLangInContext, userLanguage: userLanguage || "English" }),
            credentials: "include",
        });
        if (!response.ok) {
            const data = await response.json();
