...
- Redesigned the floating definitions options interface.
- Added American pronunciation alongside British pronunciation for English words.
- Added Cantonese pronunciation for Chinese words when displaying Traditional Chinese definitions.
- Implemented auto-close for the sentence translator when clicking outside of it.
- Removed the Pro badge from the UI in Pro options.
- Applied minor style improvements to the options page, including aligned shortcut settings.
- Fixed dictionary duplication issue for Pro users during syncing.
